...
class unetKernel(utils.KaggleKernel):
    ...
    def _dice(self, input: Tensor, targs: Tensor, iou: bool = False, eps: float = 1e-8) -> Rank0Tensor:
        n = targs.shape[0]
    ...
    def train_model(self):
        ...
        for fold in range(self.nfolds):
            ...
            learn: Learner = unet_learner(data, torchvision.models.resnext50_32x4d, callback_fns=partial(utils.CSVLoggerBufferCustomized, append=True, filename='metric.log'), cut=7, metrics=[self._dice], self_attention=True, bottle=True)
            learn.clip_grad(1.0)
            ...
            if Path('models/'+'fold' + str(fold) + 'more' + '.pth').exists():
                ...
                learn.freeze()
                learn.fit_one_cycle(4, lr, callbacks=[AccumulateStep(learn, self.n_acc)])

                # fit entire model with saving on the best epoch
                learn.unfreeze()
                learn.fit_one_cycle(4, slice(lr / 80, lr / 2), callbacks=[AccumulateStep(learn, self.n_acc)])
            ...
            # remove noise
            self.preds[self.preds.view(self.preds.shape[0], -1).sum(-1) < self.noise_th, ...] = 0.0
    ...
    def dump_state(self, exec_flag=False, force=True):  # only dataloader ... others cannot dumped
        ...
        if exec_flag:
            ...
            utils.dump_obj(data_to_save, f'run_state_{self._stage}.pkl', force=False)
    ...
    def _analyze_dice_with_threshold(self):
        ...
        plt.vlines(x=self.best_thrs[-1], ymin=self.dices.min(), ymax=self.dices.max())
        plt.text(self.best_thrs[-1] + 0.03, best_dice - 0.01, f'DICE = {best_dice:.3f}', fontsize=14);
        plt.show()
    ...
    def predict_on_test(self, thr=None):
        ...
        self.preds_test[self.preds_test.view(self.preds_test.shape[0], -1).sum(-1) < self.noise_th, ...] = 0.0
        ...
        if nc is not None:
            ...
            for i in range(self.n):
                ...
                self.factorization.append(nn.Sequential(
                    ...
                    conv2d(nc[-1]//2, nc[-1]//2, 3, padding=1, bias=True),
...
class DynamicUnet_Hcolumns(SequentialEx):
    ...
    def __init__(self, encoder: nn.Module, n_classes: int, blur: bool = False, blur_final=True,
                 ...
                 last_cross: bool = True, bottle: bool = False, small=True, **kwargs):
        imsize = (256, 256)
        ...
        if small:
            ...
            middle_conv = conv_layer(ni, ni//middle_conv_size_down_scale, **kwargs).eval()
        else:
            middle_conv_size_scale = 2
            middle_conv = nn.Sequential(conv_layer(ni, ni * middle_conv_size_scale, **kwargs),
                                        conv_layer(ni * middle_conv_size_scale, ni, **kwargs)).eval()
        x = middle_conv(x)
        ...
        for i, idx in enumerate(sfs_idxs):
            ...
            unet_block = unet_block_class(up_in_c, x_in_c, self.sfs[i], final_div=final_unet_flag,
